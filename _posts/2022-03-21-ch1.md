...
## 단일 처리기 시스템 : 초기의 시스템
## 다중 처리기 시스템 : 대부분의 최신 컴퓨터 시스템 

- 각각 단일 코어 cpu가 있는 두개 이상의 프로세서 구조 : 컴퓨터 버스, 클록, 메모리, 주변 장치 공유 일반적으로 SMP(symmetric multiprocessing)을 사용

SMP : CPU가 모든 운영체제, 사용자 프로세스 등 모든 작업을 수행하는 방식

![image](../images/ch1/11.png)

다중처리기는 발전함에 따라 다중 코어 시스템이 됨 

- 다중 코어 시스템 : 단일 칩에 여러개의 컴퓨터 코어가 들어감, 칩 내 통신이 빠름

![image](../images/ch1/12.png)

해당 그림은 이중 코어 설계 각 코어는 자체 레지스터, L1 캐시 존재


대부분의 운영체제는 다중 코어 SMP 시스템을 지원

- NUMA(non uniform memory access) : 각 CPU에 자체 로컬 메모리 제공, 모든 CPU가 공유 시스템 연결되어 하나의 물리 주소 공간을 공유 

- 장점 : 각 CPU의 로컬 메모리 접근 빠름, cpu간 경합 없음, CPU 효율적으로 추가 가능
- 단점 : 원격 메모리에 접근할때 성능저하가 발생할 수 있음 


![image](../images/ch1/13.png)
